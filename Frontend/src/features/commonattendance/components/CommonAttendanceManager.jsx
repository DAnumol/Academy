...
const CommonAttendanceManager = () => {
  ...
  const loadExistingAttendance = async () => {
    try {
      const response = await apiClient.get('/commonattendance', {
        params: {
  ...
  const handleSaveAttendance = async () => {
    ...
    try {
      ...
      await commonAttendanceService.create(attendanceData)
  ...
  const getAttendanceStats = () => {
    ...
    if (attendanceValues.length === 0) {
      return { total, present: 0, absent: 0, late: 0 }
    }
  ...
  const handleExportExcel = async () => {
    if (!filterBatch || !filterMonth || !filterYear) {
      toast.error('Please select batch, month and year')
      return
    }
    
    try {
      const response = await fetch(`http://localhost:5000/api/commonattendance/export/excel?batchId=${filterBatch}&month=${filterMonth}&year=${filterYear}`, {
        method: 'GET',
        headers: {
      ...
      a.href = url
      a.download = `Attendance_${filterMonth}_${filterYear}.xlsx`
  ...
  const handleExportPDF = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/commonattendance/export/pdf?batchId=${filterBatch}&month=${filterMonth}&year=${filterYear}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      ...
      const a = document.createElement('a')
      a.href = url
      a.download = `Attendance_${filterMonth}_${filterYear}.pdf`
      document.body.appendChild(a)
      a.click()
      window.URL.revokeObjectURL(url)
      document.body.removeChild(a)
    ...
    <div className="space-y-6">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Common Attendance</h1>
            <p className="text-gray-600 dark:text-gray-400 mt-1">Mark student attendance by batch and date</p>
          </div>
          <button
      ...
      {showFilter && (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Export Attendance Report</h2>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Batch & Date</h2>
      ...
      {isLoading ? (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-8">
          <div className="text-center">
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Choose a batch and date to mark attendance.</p>
          </div>
        </div>
      )}
    </div>
...
export default CommonAttendanceManager
